...
public class UserServiceImpl implements UserService {
	...
	public String isExistId(String id) {
		Optional<UserDTO> userDTO = userDAO.findById(id); //select * from 테이블 where id=?
		System.out.println(userDTO); //값이 없으면 Optional.empty 출력된다.
		
		if (userDTO.isPresent()) //값이 없으면 false. 있으면 true
			return "exist"; //사용 가능
		else
			return "non_exist"; // 사용 불가능
	}
	
	@Override
	public void write(UserDTO userDTO) {
		//id컬럼이 primary key로 설정되어 있기 때문에
		//똑같은 id가 없으면 insert를 수행하고, id가 있으면 update를 수행한다.
	...
	public void delete(String id) {
		...
		//내부적으로 findById() 수행, delete를 처리한다.
	...
	public Page<UserDTO> getUserSearchList(String columnName, String keyword, Pageable pageable) {
		//쿼리 메소드
		/*
		 * if(columnName.equals("name")) {//select * from usertable where name like
		 * concat('%', value, '%'); return userDAO.findByNameContaining(value); }else
		 * {//select * from usertable where id like concat('%', value, '%'); return
		 * userDAO.findByIdContaining(value); }
		 */		
		Page<UserDTO> resultPage;
		//@어노테이션(사용자 메소드)
		if(columnName.equals("name")) {
			resultPage = userDAO.getUserSearchName(keyword, pageable); // 일반 메소드
		}else {
